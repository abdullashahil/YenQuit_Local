...
import { Loader2, ChevronLeft, ChevronRight, Eye, History, Users } from 'lucide-react';
...
export function AssistHistory({ activeTab, setActiveTab }: AssistHistoryProps) {
    ...
    return (
      <div className="flex items-center justify-center py-16">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading assist history...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-[#1C3B5E] mb-2">Assist Plan History</h2>
          <p className="text-gray-600">View completed assist plans and user progress</p>
        </div>
        <div className="flex items-center gap-2 text-sm text-gray-600">
          <Users className="w-4 h-4" />
          <span>{pagination.total} total plans</span>
        </div>
      </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-start justify-between mb-4">
                <div>
                  <div className="flex items-center space-x-2 mb-2">
                    <h3 className="text-lg font-medium text-[#1C3B5E]">
              ...
              {expandedItem === item.id && (
                <div className="mt-4 pt-4 border-t border-gray-200 space-y-4">
                  {/* Coping Strategies */}
                  <div>
                    <h4 className="font-medium text-[#1C3B5E] mb-2 flex items-center gap-2">
                  ...
                  {/* Triggers */}
                  <div>
                    <h4 className="font-medium text-[#1C3B5E] mb-2">Identified Triggers</h4>
                    {item.triggers ? (
                  ...
                  {/* Notifications */}
                  <div>
                    <h4 className="font-medium text-[#1C3B5E] mb-2">Notification Preferences</h4>
                    {(() => {
                      const notifications = parseNotifications(item.notifications);
                      return notifications.length > 0 ? (
                        <div className="space-y-2">
                          {notifications.map((notif: any, index: number) => (
                            <div key={index} className="flex items-center justify-between text-sm bg-gray-50 p-3 rounded-xl">
      ...
      {pagination.pages > 1 && (
        <div className="flex items-center justify-center bg-white p-4 rounded-2xl shadow-sm">
          <div className="flex items-center space-x-2">
            <Button
              variant="outline"
              size="sm"
